...
Proportional hazards: Hazard ratios remain constant over time.
Accurate survival measurement: Event times and censoring indicators must be correctly recorded.
Independent censoring: The reason for censoring should not be related to the likelihood of the event.
Correct model specification: Relevant covariates should be included, and functional forms should be appropriate.
If these assumptions were violated, alternative approaches such as stratified Cox models or time‑dependent covariates were considered.
...
Dataset and Analytical Workflow
...
Compare survival across AJCC stages<br> Present Kaplan–Meier curves<br> Report log-rank test results<br>

\# ----------------------------- \# Create survival object \# ----------------------------- surv_object \<- Surv(cleaned_data$Survival.Months, cleaned_data$Status)
...
ggsurvplot( km_fit, data = cleaned_data, risk.table = TRUE, risk.table.height = 0.3, risk.table.fontsize = 5, pval = TRUE, conf.int = TRUE, legend.title = "AJCC Stage", xlab = "Months", ylab = "Survival Probability", palette = "Dark2", title = "Figure 5. Kaplan–Meier Survival by AJCC Stage" )
...
ggsurvplot( km_fit_er, data = cleaned_data, risk.table = TRUE, pval = TRUE, conf.int = TRUE, legend.title = "ER Status", legend.labs = c("Negative", "Positive"), title = "Figure 6. Kaplan–Meier Survival by Estrogen Receptor Status" )
...
## 5. Discussion
The Cox proportional hazards model provided clear evidence that tumor stage at diagnosis is a strong and independent predictor of survival among breast cancer patients. After adjusting for age, race, tumor size, tumor differentiation, hormone receptor status, and nodal involvement, higher AJCC 6th edition stage was associated with a significantly increased hazard of death. The magnitude and direction of the hazard ratios indicate that as tumor stage progresses, the risk of mortality rises substantially, supporting the study’s hypothesis and aligning with established clinical understanding of disease progression. These results demonstrate that tumor stage retains prognostic value even when accounting for multiple demographic and tumor‑specific characteristics.
The significance of additional covariates in the Cox model further highlights the multifactorial nature of breast cancer outcomes. Variables such as tumor size, differentiation, and nodal involvement often reflect underlying tumor biology and disease burden, and their associations with increased hazard reinforce their clinical relevance. Hormone receptor status, when significant, provides insight into tumor responsiveness to endocrine therapy and contributes to risk stratification. Importantly, the proportional hazards assumption was satisfied for the final model, indicating that the estimated hazard ratios remained stable over time and that the model appropriately captured the relationship between covariates and survival.
From a clinical perspective, these findings underscore the critical importance of early detection and accurate staging at diagnosis. The strong association between higher tumor stage and increased mortality risk highlights the need for timely screening, diagnostic evaluation, and equitable access to care. By quantifying the relative impact of stage and other clinical features on survival, the Cox model provides valuable information that can support individualized treatment planning and follow‑up strategies. Overall, this analysis reinforces the central role of tumor stage in predicting breast cancer survival and demonstrates the utility of the Cox proportional hazards model in evaluating complex clinical datasets.

# 6. Conclusion

This study used the Cox proportional hazards model to evaluate how tumor stage at diagnosis influences survival among breast cancer patients. The results demonstrated that higher AJCC stage is strongly associated with an increased hazard of death, even after adjusting for demographic and tumor‑specific covariates such as age, race, tumor size, differentiation, hormone receptor status, and nodal involvement. These findings support the original hypothesis and reinforce the central role of tumor stage as a key prognostic factor in breast cancer survival.
By quantifying the relative risk associated with each stage and relevant clinical features, the Cox model provided a clear and interpretable framework for understanding how disease severity at diagnosis affects patient outcomes. Clinically, these results highlight the importance of early detection, timely diagnosis, and equitable access to screening and treatment services. Overall, this analysis contributes to a deeper understanding of survival patterns in breast cancer and underscores the continued value of the Cox proportional hazards model in evaluating complex clinical datasets.
...
Smith, T., Smith, B., & Ryan, M. A. (2003, March). Survival analysis using Cox proportional hazards modeling for single and multiple event time data. In Proceedings of the twenty-eighth annual SAS users group international conference, SAS Institute, Inc, Cary, paper (pp. 254-228).
